package kz.rymbek.platform.common.base.feature.extensions

object ListStateExtensions {
    fun <T> List<T>.addItem(path: T, maxItems: Int = 30): List<T> = if (size < maxItems) this + path else this

    fun <T> List<T>.editItem(index: Int, path: T): List<T> = if (index in indices) {
        toMutableList().apply { this[index] = path }
    } else {
        this
    }

    fun <T> List<T>.deleteItem(index: Int): List<T> = if (index in indices) {
        toMutableList().apply { removeAt(index) }
    } else {
        this
    }

    fun <T> Set<T>.toggle(value: T): Set<T> = if (contains(value)) this - value else this + value

    fun <T> T?.toggle(newValue: T): T? = if (this == newValue) null else newValue

    fun <T> List<T>.toggle(value: T): List<T> = if (contains(value)) this - value else this + value
}
